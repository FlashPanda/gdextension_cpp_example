...
在 GREEN 后、最终验证前的 REFACTOR 阶段执行本方案。让注释解释代码本身无法稳定表达的约束，不复述语句表面含义。

#### 注释判定

- 必须注释非显而易见的设计原因、算法不变量、数值单位或坐标空间，以及违反约束的后果。
- 必须注释跨模块数据流、降级策略、失败路径和调用方不能从接口直接推断的前置/后置条件。
- 必须注释并发边界：主线程与 worker 的职责、共享或独占数据、同步方式、取消点、join 时机，以及 Godot Object 的访问限制。
- 必须注释资源边界：创建者、所有者、生命周期、释放方和失败后的清理责任。
- 生成的代码注释必须使用中文；标识符、类型、公式、坐标空间名称和必要的代码术语可以保留英文。
- 仅当名称和签名不足以说明职责时，才为模块、类或函数补充摘要；简单 getter、直观分支和机械转换不写逐行注释。
- 禁止保留注释掉的代码、复述实现的注释和仅记录修改历史的注释；历史放入需求文档或版本记录。
- `TODO` 必须写成 `TODO(REQ-XXX)`，同时说明剩余工作和完成条件；没有活动需求编号时不要新增 `TODO`。

#### 生成流程

1. 在 REFACTOR 阶段检查本次 diff、相关声明和直接调用方，圈定他人无法在 30 秒内从代码推断的约束。
2. 按“意图/不变量、数据/单位、失败、线程、所有权/生命周期”逐项判断，只为实际存在的复杂度生成注释。
3. 把注释放在能够约束目标代码的最窄稳定位置：优先放在声明或代码块之前，避免散落到每条语句后。
4. 强制使用中文生成代码注释；标识符、类型、公式、坐标空间名称和必要的代码术语保持英文，并按需使用反引号标记。
5. 写清“为什么、必须保持什么、边界在哪里、违反后会怎样”，不把期望行为只写在注释里而缺少代码或测试保证。
6. 同步更新或删除受改动影响的旧注释，再执行完成检查；注释与实现冲突时按缺陷处理。

#### 紧凑模板

- 算法：`// <为何采用此策略>；保持 <不变量>，否则 <后果>。`
- 数据：`// 输入为 <单位/坐标空间>；输出为 <单位/坐标空间>；无效数据时 <降级/失败行为>。`
- 并发：`// 线程边界：<谁执行>；<数据> 由 <谁> 独占/共享，通过 <同步方式> 保护；在 <时机> 取消/join。`
- 资源：`// 生命周期：由 <创建者> 创建，<所有者> 持有，<释放方> 在 <时机> 释放；失败时 <清理方式>。`
- 待办：`// TODO(REQ-XXX): <剩余工作>；完成条件：<可验证结果>。`

渲染与路径追踪优先说明 seed、PDF、throughput、法线、ray epsilon、累积规则、单位和坐标空间；场景提取优先说明 world/local 变换、normal inverse-transpose、资源快照和 fallback；异步导出优先说明主线程/worker 边界、不可变快照、所有权、取消、join 和文件失败路径。

#### 完成检查

- 确认复杂改动已按需覆盖意图、数据流、失败路径、线程边界和资源生命周期，且每条注释都与当前实现一致。
- 确认本次新增或修改的代码注释全部使用中文，仅在必要位置保留英文代码元素。
- 确认 worker 不访问未明确证明线程安全的 Godot Object，并在必要位置写明主线程交接点。
- 确认没有冗余逐行解释、过期注释、注释掉的代码或缺少活动需求编号及完成条件的 `TODO`。
- 缺少必需注释或存在误导注释时，不得把需求标为完成。

### 风格

- 使用 C++17，保持现有命名、目录和 include 风格。
- 禁止任意形式的重构 `godot-cpp/`。
- 遵循“注释生成方案”；复杂代码的必要注释属于完成门槛。
